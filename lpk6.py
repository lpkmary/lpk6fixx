...
if test == "Home👩🏻‍🔬":
    ...
    def cariHasil(asamKuat, basaKuat):
        if asamKuat == 'H2SO4':
            asam = asamKuat[1:]
            basa = basaKuat.split("O")[0]
            basatemp = "2"+ basaKuat
            left_side = f"{basa}{asam}"
            right_side = " + 2H2O"
            hasil = left_side + right_side
            st.success(f"Hasil reaksi dari {asamKuat} + {basaKuat} -> {asamKuat} + {basatemp} = {hasil}")
        else:
    ...
    with tab3:
        ...
        st.write('Reaksi antara asam kuat dengan basa kuat akan menghasilkan garam dan air. pH larutan yang dihasilkan bersifat netral atau pH = 7')
...
if test == "About Asam Kuat dan Basa Kuat🧪":
    ...
    with tab1:
        ...
        text = """
        Asam basa Arrhenius menyatakan bahwa asam adalah zat yang apabila dilarutkan dalam air akan menghasilkan ion H+ dalam larutan dan basa adalah zat yang apabila dilarutkan dalam air akan menghasilkan ion OH– dalam larutan. Dari pengertian tersebut, bisa disebutkan ciri khas asam adalah apabila dalam pelarut air, zat akan mengion menjadi hidrogen dengan muatan positif dan ion yang bermuatan negatif tersebut adalah sisa asam. Lalu, ciri khas basa adalah apabila dalam pelarut air, zat akan mengion menjadi ion hidroksida yang muatannya negatif dan ion bermuatan positif disebut sisa basa.
        """
        html_content = f"""
        <div style="text-align: justify;">
            {text}
        ...
        text = """
        Asam kuat adalah jenis asam yang dapat sepenuhnya terionisasi atau berdisosiasi dalam larutan air, menghasilkan ion hidrogen (H⁺) dalam jumlah besar."""
        html_content = f"""
        <div style="text-align: justify;">
            {text}
        </div>
        ...
        text = """ Asam Klorida (HCl): Merupakan gas yang larut dalam air membentuk larutan asam kuat. Asam Sulfat (H₂SO₄): Asam kuat ini digunakan secara luas dalam industri dan laboratorium.
        """
        html_content = f"""
        <div style="text-align: justify;">
            {text}
        ...
        text = """ Basa kuat adalah jenis basa yang dapat sepenuhnya terdisosiasi dalam larutan air, menghasilkan ion hidroksida (OH⁻) dalam jumlah besar.
        """
        html_content = f"""
        <div style="text-align: justify;">
            {text}
        ...
        text = """Natrium Hidroksida (NaOH): Basa kuat yang larut dalam air dan digunakan dalam berbagai aplikasi industri. Kalium Hidroksida (KOH): Basa kuat ini juga digunakan dalam berbagai proses industri."""
        html_content = f"""
        <div style="text-align: justify;">
            {text}
        </div>
        ...
        text = """Asam lemah adalah senyawa yang dilarutkan dalam air akan sulit melepaskan ion H+ dan mengalami disosiasi pada larutan. Sedangkan, basa lemah adalah senyawa yang apabila dilarutkan di dalam air akan sulit melepaskan ion OH- dan mengalami disosiasi dalam larutan."""
        html_content = f"""
        <div style="text-align: justify;">
            {text}
        </div>
